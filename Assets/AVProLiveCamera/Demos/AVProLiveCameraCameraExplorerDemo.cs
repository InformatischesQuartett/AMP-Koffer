...
public class AVProLiveCameraCameraExplorerDemo : MonoBehaviour
{
	...
	public void Update()
	{
		...
		if (frameCt % 180 == 0) {
			curmode++;
						AVProLiveCameraDevice device = AVProLiveCameraManager.Instance.GetDevice (2);
			AVProLiveCameraDeviceMode mode = device.GetMode (curmode);
			Debug.Log ("" + mode.Width + "x" + mode.Height + " " + mode.FPS.ToString ("F2") + "hz " + "[" + mode.Format + "]");
					
						device.Close ();
						device.Start (curmode);

				
			
			Debug.Log ("mode: " + curmode);
		}
